...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.fps = 400 #cap.get(cv2.CAP_PROP_FPS) % 100

            #self.imgs[i] = self.patch() #cap.read()  # guarantee first frame
            
            thread = Thread(target=self.patch, args=([i, 1]), daemon=False).start()
            while True:
                if self.ffmpeg==1:
                    break
                time.sleep(1/60)
    ...
    def patch(self,indexasdasd,a=1):
        ...
        pipe = sp.Popen(ffmpeg_cmd, stdout = sp.PIPE, bufsize=100000)
